...
Recorded final validation is green: mobile Vitest **87 files / 1208 tests passed**; mobile static server **6/6 passed**; API **36 files passed + 1 skipped / 457 passed + 4 skipped**; Calora, API, and library typechecks passed; and `git diff --check` passed. There are no remaining source P0, P1, or P2 blockers. Physical-device revalidation remains required.
...
| 7 | Discover/Plus repeatedly showed the same top recipes. | P2 | Account-and-UTC-day stable rotation for unfiltered Discover; Plus preserves account-scoped loaded inventory and provider pagination. |
...
7. **Freshness:** default Discover used stable provider ordering, so the first slice repeated. Naive render-time randomization would have broken pagination. Search and category results already had relevance/provider ordering and therefore must not be shuffled.
...
**Verified source behavior:** loaded Plus recipes are lifted into `RecipesScreen` as `premiumCatalogueState`, tagged with the current `userId`, and passed back when the Plus child is shown again. Existing cards therefore remain immediately available across submenu remounts rather than blanking while a request runs. The React Query key remains account-aware. On identity change, lifted data is reset and `['premium-recipes']` queries are removed, preventing cross-account contamination.

`samePremiumCatalogueState` suppresses identical parent writes by comparing owner, length, recipe IDs, and images, preventing the child-publish/parent-render loop. Existing content remains while background/error UI is handled separately. Scroll position is retained by the screen's per-section scroll refs where the existing architecture permits it.
...
**Verified source behavior:** the server now calculates provider-page boundaries independently from the count that survives normalization and deduplication. It returns an advancing `nextOffset` or `null`, plus an explicit `terminalReason`. This avoids premature exhaustion when a provider page contains invalid/duplicate rows.

The client applies a page only when it is not React Query placeholder data. It appends only unique IDs, keeps existing rows visible, blocks re-entry while fetching, and stops on explicit terminal reason, missing cursor, zero unique append with a next cursor, or a non-advancing cursor. Near-bottom/content-size triggers continue pagination, loading and retry footers remain available, and terminal exhaustion is shown instead of silently looping. No recipes are fabricated.
...
**Verified algorithm:** only **unfiltered Discover** is rotated. The API obtains the server-verified account ID when available (otherwise `guest`) and combines it with the current **UTC day** (`YYYY-MM-DD`). It SHA-256-ranks each recipe ID against that seed, with recipe ID as a tie-breaker, before slicing offsets.

Consequences:

- one account has a stable order for the whole UTC day, so pagination and caching do not reshuffle mid-scroll;
- different accounts and subsequent UTC days can receive different top cards;
...
Plus does not use unsafe render-time randomization. Its freshness improvement is retention of the current account's loaded catalogue plus correct continued provider pagination/background fetch, rather than repeatedly resetting to the same first page. This is an honest distinction: the account+UTC-day rotation is specifically the default Discover pool.

**Test evidence:** API recipe tests verify deterministic same-seed ordering, seed/account/day variation, pagination validity, uniqueness, and preservation of search/category ordering.
...
| Validation | Result |
|---|---|
| Calora mobile Vitest | **87 files passed; 1208 tests passed; 0 failures** |
| Mobile static server suite | **6/6 passed; 0 failures** |
| API Vitest | **36 files passed + 1 skipped; 457 tests passed + 4 skipped; 0 failures** |
...
6. Open/leave/reopen Plus; confirm immediate retained cards and scroll behavior. Continue through multiple pages and true provider exhaustion without blanking, duplicates, or loops.
...
- **Release evidence blocker:** owner physical-device revalidation is required, especially for onboarding replay, keyboard geometry, Camera-to-Today process/offline durability, live Plus pagination, and native health permissions.
...
The targeted current-source remediation is complete, deterministic validation is green, and no source P0/P1/P2 blocker remains. The result is not a physical-device certification. No build, Expo/EAS action, workflow restart, push, publish, DNS/provider configuration change, or production deletion was performed. Owner validation of the exact installed-device flows is the remaining release gate.
