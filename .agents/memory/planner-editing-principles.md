...
The Planner should make editing visible before the user acts: an explicit Edit/Done mode exposes direct meal edits, while overflow actions remain available for move, copy, replace, remove, and diary review. Custom meals and serving changes must use the same shared planner mutation path so Shopping, adaptive memory, and summaries stay synchronized. Destructive removals should offer a visible, time-bounded Undo action in the save acknowledgement and restore the exact prior slot through that same mutation path. Move/copy must refuse an occupied destination instead of silently deleting it, and every replacement must use a new planner identity while explicitly preserving diary linkage.

**Why:** Hidden or fragmented edit behavior makes a planning workspace feel administrative and can leave downstream surfaces out of sync. A recovery action that disappears too quickly or restores through a separate path risks making an otherwise safe removal feel like data loss. Reusing a planner ID for a replacement can also make a historical diary entry appear to change meals retroactively.
