...
| F-14 | In-app invite handoff and universal links | BLOCKED | App route persists code then sends signed-out users to sign-up; automated persistence tests pass. The published origin returns `200` association files. | Install a new signed build containing `calorie-coach-pie35449.replit.app`, then complete Apple/Android OS verification and force-quit testing. |
| F-15 | Universal-link verification files | PASSED (published service) | Both association endpoints return `200` with configured identifiers over production TLS. | Direct OS handoff still requires the new native build and device association-cache verification. |
...
2. Install a new signed build containing the published Replit host, re-verify both `/.well-known/` files, then test installed-app and no-app invite routes on iOS and Android.
