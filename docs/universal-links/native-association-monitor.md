...
provider migration window with the repository Actions variable
`NATIVE_ASSOCIATION_FRESHNESS_MAX_AGE_SECONDS`. Values must be whole seconds
between `3600` (one hour) and `604800` (seven days), inclusive. An unset,
malformed, or out-of-range value fails closed to the default `86400` seconds
(24 hours); the monitor reports only the effective threshold and whether it was
configured or fell back to the default.
...
NATIVE_ASSOCIATION_FRESHNESS_MAX_AGE_SECONDS='86400' \
