...
## Appendix A. Publish-history evidence record

This appendix consolidates the former standalone Publish History Evidence
report into this canonical governance record.
...
Deployment logs also record artifact startup for the corresponding publication
...
The post-publish read-only managed PostgreSQL comparison reports all four
Phase 2B tables:
...
The ordered production metadata fingerprint is:
