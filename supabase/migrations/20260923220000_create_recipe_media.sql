...
CREATE TABLE IF NOT EXISTS public.calora_recipe_media (
...
CREATE UNIQUE INDEX IF NOT EXISTS calora_recipe_media_owner_recipe_hash_idx
  ON public.calora_recipe_media (owner_external_id, client_recipe_id, content_hash);
CREATE UNIQUE INDEX IF NOT EXISTS calora_recipe_media_owner_image_idx
  ON public.calora_recipe_media (owner_external_id, image_id)
  WHERE image_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS calora_recipe_media_owner_updated_idx
  ON public.calora_recipe_media (owner_external_id, updated_at DESC);

ALTER TABLE public.calora_recipe_media ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.calora_recipe_media FROM anon, authenticated;

COMMENT ON TABLE public.calora_recipe_media IS
...
COMMENT ON COLUMN public.calora_recipe_media.semantic_review_state IS
