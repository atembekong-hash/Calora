...
export class RecipeFreshnessSession {
  ...
  order<T extends RecipeIdentity>(recipes: readonly T[], visit = this.visit, now = Date.now()): T[] {
    ...
    // Provider ordering remains deterministic. When a small pool is entirely
    // recent, rotate it by the bounded visit count rather than fabricating rows.
...
/**
 * Session-only freshness is deliberately account and surface scoped by the
 * caller. It never writes recipe history to durable storage.
 */
