...
export function orderProgramMeals<T extends ProgramEligibleMeal>(
  programId: PlannerProgramId | undefined,
  meals: T[],
): T[] {
...
export function selectDiverseProgramMeal<T extends ProgramEligibleMeal>(
  ...
  return candidates.length === 0 ? undefined : candidates[dayIndex % candidates.length];
}