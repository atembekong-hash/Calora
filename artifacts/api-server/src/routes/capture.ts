...
/**
 * Marks a server-issued review proof as approved. This remains separate from
 * diary syncing: offline diary logging must not depend on this request, while
 * existing clients still use it to record an explicit review decision.
 *
 * Approval is owner-scoped and idempotent. It only changes a session from
 * review → approved; a retry for the same owner succeeds without changing
 * any later state. `reviewedAt` remains reserved for the capture-first diary
 * sync claim path.
 */
router.post("/v1/capture/:sessionId/approve", async (req, res) => {
  ...
  try {
    ...
    if (classifyAccountDeletionError(error)) {
      logger.warn(
        ...
        "Account deletion fence rejected capture approval",
      );
  ...
  try {
    ...
    // Do not reveal whether a session is missing or belongs to another
    // account. An already-approved session owned by this caller is the only
