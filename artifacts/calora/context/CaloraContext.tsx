...
export type FoodSource = 'USDA verified' | 'Brand verified' | 'Restaurant verified' | 'Barcode verified' | 'Photo estimate' | 'Manual' | 'Recipe';
...
export function CaloraProvider({
  ...
  const startLiveStepTracking = useCallback(async (requestPermission = false) => {
    ...
    if (tracking.day !== currentDay) {
      tracking = createLiveStepTrackingState(currentDay, healthConnectionRef.current.snapshot?.steps ?? null);
    ...
    try {
      ...
      liveStepSubscriptionRef.current = stepMotionService.watchSteps((steps) => {
        ...
        commitLiveStepTracking(projectLiveSteps(current, steps, new Date().toISOString()));
      });
    } catch (error) {
      if (!isCurrent()) return;
  ...
  }, [commitLiveStepTracking, syncHealth]);
  ...
  const value = useMemo<CaloraContextValue>(() => ({
    ...
    acceptFoodMemory: (draftId, draftOverride) => {
      ...
      // Prefer the synchronous collection to the render closure. The override
      // remains a direct handoff for a draft created in the same event turn.
      ...
      return coordinateCaptureAcceptance(acceptingFoodDraftsRef.current, draftId, async (): Promise<FoodLog | null> => {
        ...
        const nextLivingMemory = upsertMealObservation(
          exportSnapshotRef.current.livingMemory as LivingMemory,
          log.id,
          log.date,
          log.meal,
        );
        ...
        const persistedSnapshot = {
          ...exportSnapshotRef.current,
          logs: nextLogs,
          foodDrafts: nextDrafts,
          foodMemories: nextMemories,
          repeatPatterns: nextRepeatPatterns,
          livingMemory: nextLivingMemory,
          outbox: nextOutbox,
        };
