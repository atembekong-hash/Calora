...
function printFailure(reason, bundleIdentifier) {
  console.error('\n[ios-signing] RELEASE PREFLIGHT FAILED');
  console.error(`[ios-signing] ${reason}`);
...
function runEasBuild() {
  ...
  if (result.error) {
    console.error(`[ios-signing] Unable to start ${command}: ${result.error.message}`);
...
async function main() {
  ...
  try {
    ...
    printFailure(error.message, 'unknown');
    return 1;
  }
  ...
  try {
    ...
    printFailure(
      `Could not read EAS iOS signing status: ${error.message}`,
      identity.bundleIdentifier,
    );
    return 1;
  }
  ...
  if (!readiness.ready) {
    printFailure(readiness.reason, identity.bundleIdentifier);
    return 1;
  }
  ...
  if (process.argv.includes('--queue-build')) {
    console.log('[ios-signing] Credentials are ready; queuing the current revision with EAS...');
    return runEasBuild();
  }
