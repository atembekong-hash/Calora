  ...
  return <View style={{ height }}>
    ...
    <View style={[styles.recipeImage, styles.imageFallback, { height }]}>
      ...
      <View style={styles.imageFallbackCopy}>
         <Feather name="image" size={22} color="#d4eadc" />
...
function RecipeMeta({ recipe, colors, compact = false }: { recipe: BrowseRecipe; colors: ReturnType<typeof useCalora>['colors']; compact?: boolean }) {
  ...
  const estimated = provenance.nutritionConfidence === 'estimated';
  const hasCalories = recipe.calories != null;
  // Never imply a nutrient value that was not supplied. The provenance label is
  // deliberately separate from the values so partial provider nutrition stays
  // truthful instead of looking like a complete macro calculation.
  const nutrition = hasCalories ? `${estimated ? '~' : ''}${formatCalories(recipe.calories)}` : recipeNutritionLabel(recipe);
  return (
    <View style={[styles.recipeMeta, compact && styles.compactRecipeMeta]}>
      <Text style={[styles.recipeKcal, { color: hasCalories ? colors.foreground : colors.warning }]}>{nutrition}</Text>
      {recipe.proteinG != null ? <Text style={[styles.recipeMetaText, { color: colors.mutedForeground }]}>{estimated ? '~' : ''}{formatGrams(recipe.proteinG)} P</Text> : null}
      <Text style={[styles.recipeMetaText, { color: colors.mutedForeground }]}>{recipeNutritionLabel(recipe)}</Text>
...
export function RecipeCard({ recipe, colors, saved, onPress, onSave, imageHeight = 160, remainingCalories, fixedHeight, compact = false }: { recipe: BrowseRecipe; colors: ReturnType<typeof useCalora>['colors']; saved: boolean; onPress: () => void; onSave: () => void; imageHeight?: number; remainingCalories?: number; fixedHeight?: number; compact?: boolean }) {
  ...
  return (
    <Surface tier="flat" radius="lg" style={[styles.recipeCard, fixedHeight ? { height: fixedHeight } : null]}>
      <View style={fixedHeight ? styles.recipeCardInner : undefined}>
        <Pressable
          accessibilityRole="button"
          accessibilityLabel={`Open ${recipe.name}`}
          onPress={onPress}
          style={({ pressed }) => [fixedHeight ? styles.recipeCardPressable : null, { opacity: pressed ? 0.96 : 1 }]}
        >
...
function PremiumCatalogue({ colors, visible, onOpen, onSave, savedPremiumRecipes, initialState, onLoadMoreRef, onLoadedRecipesChange, onScrollYChange, onProtectedStateClear }: { colors: ReturnType<typeof useCalora>['colors']; visible: boolean; onOpen: (recipe: PremiumRecipe) => void; onSave: (recipe: PremiumRecipe) => void; savedPremiumRecipes: PremiumRecipe[]; initialState: PremiumCatalogueState; onLoadMoreRef: React.MutableRefObject<(() => void) | null>; onLoadedRecipesChange: (state: PremiumCatalogueState) => void; onScrollYChange: (scrollY: number) => void; onProtectedStateClear: () => void }) {
  ...
  // `placeholderData` is the previous offset's response. It is useful for a
  // non-jumping grid, but it is not evidence about the requested page cursor.
  ...
  return (
    ...
    >
      <View testID="plus-recipe-catalogue">
        ...
        {paginationTerminalReason && (
          <Text testID="plus-recipe-pagination-terminal" style={[styles.sectionCaption, { color: colors.mutedForeground }]}>
            {paginationTerminalReason}
          </Text>
        )}
        {!paginationTerminalReason && hasCurrentPageData && data?.terminalReason && (
          <Text testID="plus-recipe-pagination-terminal" style={[styles.sectionCaption, { color: colors.mutedForeground }]}>
            {data.terminalReason}
          </Text>
        )}
...
export function RecipeDetailModal({ recipe, onClose, onPlanned, onRetryPhoto, onPremiumAccessDenied, suggestedRecipes = [], onSelectSuggestion }: { recipe: Recipe | CaloraRecipe | null; onClose: () => void; onPlanned: (message: string) => void; onRetryPhoto: (recipe: CaloraRecipe) => void; onPremiumAccessDenied?: () => void; suggestedRecipes?: BrowseRecipe[]; onSelectSuggestion?: (recipe: BrowseRecipe) => void }) {
  ...
  const acceptDraft = async () => {
    if (!reviewDraft) return;
    await acceptFoodMemory(reviewDraft.id);
...
export default function RecipesScreen() {
  ...
  const [premiumCatalogueState, setPremiumCatalogueState] = useState<PremiumCatalogueState>({ userId: null, recipes: [] });
  const [nextRemoteOffset, setNextRemoteOffset] = useState<number | null>(0);
  const [remoteTerminalReason, setRemoteTerminalReason] = useState<string | null>(null);
  const loadingMoreRef = useRef(false);
  ...
  const localMatches = useMemo(() => category !== 'My recipes' ? [] : localRecipes.filter((recipe) => {
  ...
  const recipesQuery = useListRecipes(
    { query: search || undefined, category: category === 'For you' || category === 'My recipes' || category === 'Quick' ? undefined : category, limit: RECIPE_PAGE_SIZE, offset: remoteOffset },
    {
      query: {
        queryKey: ['recipes', search, category, remoteOffset],
        staleTime: 1000 * 60 * 10,
        retry: false,
  ...
  useEffect(() => {
    setRemoteOffset(0);
    setRemoteRecipes([]);
    setNextRemoteOffset(0);
    setRemoteTerminalReason(null);
    loadingMoreRef.current = false;
  }, [search, category]);
  useEffect(() => {
    ...
    setRemoteRecipes((current) => {
      ...
      // Discover is an intentionally continuous browse feed. The API cycles
      // its deterministic provider pool after the first pass, so do not treat
      // repeated IDs at a cycle boundary as exhaustion.
      return [...current, ...page];
    });
    const nextOffset = recipesQuery.data?.nextOffset ?? null;
    setNextRemoteOffset(nextOffset !== null && nextOffset > remoteOffset ? nextOffset : null);
    if (nextOffset !== null && nextOffset <= remoteOffset) setRemoteTerminalReason('The recipe source cannot provide another page.');
    loadingMoreRef.current = false;
  ...
  const recipeSuggestions = useMemo(() => {
    ...
    const pool: BrowseRecipe[] = premiumSelected
      ? premiumCatalogueState.recipes
      : [...visibleLocal, ...visibleRemote];
    return getRecipeSuggestions(pool, selectedRecipe);
  }, [premiumCatalogueState.recipes, selectedRecipe, visibleLocal, visibleRemote]);
  ...
  const loadMoreRecipes = () => {
    if (activeSection !== 'discover' || category === 'My recipes' || nextRemoteOffset == null || remoteTerminalReason || recipesQuery.isFetching || loadingMoreRef.current) return;
    loadingMoreRef.current = true;
    setRemoteOffset(nextRemoteOffset);
  };
...
function makeStyles(f: number) {
  ...
  cardContent: { padding: 10, flex: 1, justifyContent: 'space-between' },
   compactCardContent: { paddingHorizontal: 8, paddingTop: 7, paddingBottom: 6 },
   compactRecipeMeta: { marginTop: 4 },
  ...
  cardFooter: { flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', marginTop: 9, paddingTop: 8, borderTopWidth: 1, borderTopColor: 'rgba(120,120,120,0.12)' },
   compactCardFooter: { marginTop: 5, paddingTop: 5 },
